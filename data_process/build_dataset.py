"""
modified 1 :从一个文件夹中读取批量csv
modified 2 :对每个文件夹的600 frame 左右生成很多 pt文件
to modified 3 每个pt文件 预测时长从3s变成 5s [为了验证方便 目前还未修改]
"""
import os
...
if __name__ == '__main__':
    ...
    for i in range(0, len(csv_files), 2):
        ...
        for frame_to_get in range(frame_start,frame_end,10):
            ...
            torch.save(_data, os.path.join(pt_folder_path, prefix + str(frame_to_get) + '.pt'))# 根据目录存为pt文件
